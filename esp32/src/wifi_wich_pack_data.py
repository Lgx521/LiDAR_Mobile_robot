...
def pack_data(raw_data):
    # 雷达协议示例：0xAA 0xAA [长度] [数据] [CRC32]
    header = b'\x54\x2c'
    length = len(raw_data).to_bytes(2, 'big')
    crc = crc32(raw_data).to_bytes(4, 'big')
    return header + length + raw_data + crc
        ...
        try:
            ...
            if client_sock:
                ...
                if uart.any():
                    ...
                    try:
                        client_sock.sendall(pack_data(data))
                    except:
                        print('error')
                        client_sock.close()
                        client_sock = None
